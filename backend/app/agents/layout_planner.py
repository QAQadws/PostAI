...
class SpatialLayoutPlanner:
    ...
    def _build_messages(self, state: GraphState) -> list[dict]:
        ...
        system = (
            ...
            "- Unless the brief explicitly asks for extreme minimalism, include "
            "at least 5 visible layers: base field, texture/noise/grid, large "
            "cropped type or symbol, directional shape, focal visual, metadata "
            "strip, frame/rules, and foreground text hierarchy.\n"
            "- Avoid a single centered icon, one small SVG circle, or one gradient "
            "background as the entire visual idea. If you use abstract geometry, "
            "make it purposeful, scaled, cropped, and layered.\n"
